...
## IGNORE: Invalid Svelte store syntax on objects containing multiple stores

**- Pattern:** Using the `$` syntax on a plain object that contains multiple stores (e.g. `$doomfireStoreDecay` after `const { decay } = doomfireStore`).
**- Justification:** While the project prefers idiomatic Svelte reactive store syntax (`$storeName`) over manual `.subscribe()` calls, this strictly applies to actual store instances, not to objects wrapping multiple stores. Destructuring and directly accessing stores through `$` syntax on sub-properties fails in Svelte.
**- Files Affected:** `src/App.svelte`, `src/pages/DoomfireControl.svelte`
