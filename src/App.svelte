<script lang="ts">
    ...
    const { decay: doomfireStoreDecay, wind: doomfireStoreWind } = doomfireStore;
    ...
    let doomfireContainerRef: HTMLElement;
...
<div class="doomfire-container" bind:this={doomfireContainerRef} on:click={handleJump("/")} on:keypress={noop} role="button" tabindex="0">
    <!-- {#if $isUserInteractedStore } -->
        <DoomFire
            on:render={() => console.log('doomfire render')}
            on:resize={() => console.log('doomfire resize')}
            containerRef={doomfireContainerRef}
            decay={$doomfireStoreDecay}
            wind={$doomfireStoreWind}
        />
    <!-- {/if} -->
</div>
<audio bind:this={musicRef} id="audio-intro" src="/intro.m4a" loop></audio>
<main on:click={handleUserInteraction} on:keypress={noop} tabindex="-1">
