...
export function getProblemAnswer(problem: Problem): number {
...
const alternativeStrategies: Array<(problem: Problem) => number> = [
  ({ a, b, op }) => {
    ...
    const newA = Math.round(
      a * (1 - variationPercent + Math.random() * variationPercent * 2),
    );
  ...
  ({ a, b, op }) => {
    ...
    const newB = Math.round(
      b * (1 - variationPercent + Math.random() * variationPercent * 2),
    );
  ...
  ({ a, b, op }) => {
    ...
    return Math.round(
      answer * (1 - variationPercent + Math.random() * variationPercent * 2),
    );
  },
  ({ a, b, op }) => {
...
export function generateAlternatives(problem: Problem, amount = 4): number[] {
  const solution = getProblemAnswer(problem);
  let alternatives = new Set<number>();
  let i = 0;
  while (alternatives.size < amount) {
    const strat =
      alternativeStrategies[
        Math.floor(Math.random() * alternativeStrategies.length)
      ];
  ...
  while (alternatives.size < amount) {
    i++;
    if (i > 200) break;
    alternatives.add(getProblemAnswer(generateProblem({ max: 100 })));
